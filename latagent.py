...
def pick_label_column(df: pd.DataFrame, label_cols: list):
    ...
    Trả về (tên_cột_hiển_thị, Series_giá_trị_dạng_chuỗi) hoặc (None, None) nếu không có gì phù hợp.
    """
    if not label_cols:
        return None, None
    ...
    return chosen, df[chosen].astype(str)
...
def render_smart_chart(df: pd.DataFrame, chart_override: str, turn_id: str):
    ...
    try:
        ...
        if chosen == "Line" and time_col and measure_cols:
            ...
            label_name, label_series = pick_label_column(df, label_cols)
            if label_name is None:
                st.info("Không tìm thấy cột phù hợp để làm nhãn trục X.")
                return
            plot_df = df.copy()
            plot_df[label_name] = label_series.values
            # Ép trục X thành danh mục (category) tường minh + giữ đúng thứ tự xuất hiện
            # trong dữ liệu (VD: Top-N đã ORDER BY giảm dần từ SQL), tránh bị Plotly
            # hiểu nhầm thành trục số liên tục hoặc tự sắp lại theo alphabet.
            category_order = list(dict.fromkeys(label_series.tolist()))
            fig = px.bar(
