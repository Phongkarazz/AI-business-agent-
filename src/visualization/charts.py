                    ...
                    # Ưu tiên vẽ Dual Axis khi người dùng hỏi so sánh Doanh số ($) và Số lượng hộp bán ra giữa các Team / đối tượng kinh doanh
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                mode="lines+markers+text",
                                marker=dict(size=12, color="#FF7A00", line=dict(width=2, color="#FFFFFF")),
                                line=dict(width=3.5, color="#FF7A00"),
                                text=plot_df[boxes_c],
                                texttemplate="%{text:,.0f} hộp",
                                textposition="top center",
                                cliponaxis=False,
                                textfont=dict(color="#B45309", size=13, family="sans-serif"),
                            ),
                            secondary_y=True,
                        )
                        try:
                    ...
                    # Giúp hiển thị trực quan cả quy mô quỹ lương lẫn mức thu nhập bình quân/nhân sự mà không bị lệch thang đo (Visual Scale Contradiction)
                        ...
                        fig.add_trace(
                            go.Bar(
                                ...
                                insidetextanchor="middle",
                                textfont=dict(color="#FFFFFF" if len(plot_df) <= 12 else "#1E40AF", size=12, family="sans-serif"),
